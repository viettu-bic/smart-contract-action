...
contract BicAccountFactory {
    ...
    function createAccount(address owner, uint256 salt) public returns (BicAccount ret) {
        ...
        ret = BicAccount(
            payable(
                new ERC1967Proxy{salt: bytes32(salt)}(address(accountImplementation), abi.encodeCall(BicAccount.initialize, (owner, permissions)))
            )
        );
    }
            ...
            Create2.computeAddress(
                ...
                keccak256(
                    abi.encodePacked(
                        ...
                        abi.encode(address(accountImplementation), abi.encodeCall(BicAccount.initialize, (owner, permissions)))
                    )
                )
            );
    }
