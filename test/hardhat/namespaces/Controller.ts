...
describe("Controller", function () {
  ...
  beforeEach(async () => {
    ...
    usernameHandles = await ethers.getContractAt(
      "Handles",
      cloneAddress as any
    );
    ...
    const BicTokenPaymaster = await ethers.getContractFactory(
      "BicTokenPaymaster"
    );
    bic = await BicTokenPaymaster.deploy(entryPoint.target, deployer.address);

    const HandlesController = await ethers.getContractFactory(
      "HandlesController"
    );
  ...
  it("Controller: create nft directly", async function () {
    ...
    await bic
      .connect(wallet1)
      .approve(handlesController.target, ethers.parseEther("1"));
    ...
    const txRequestHandle = handlesController.connect(wallet1).requestHandle(
      {
        receiver: wallet1.address,
        handle: usernameHandles.target,
        name: mintName,
    ...
    await expect(txRequestHandle)
      .to.emit(handlesController, "MintHandle")
      .withArgs(
        usernameHandles.target,
        wallet1.address,
        mintName,
        price,
        BigInt(MintType.DIRECT)
      );
    ...
    expect(initialBicBalance - price).to.equal(
      await bic.balanceOf(wallet1.address)
    );
    expect(await bic.balanceOf(wallet2.address)).to.equal(price / 10n);
    expect(await bic.balanceOf(randomWalletAddress)).to.equal(
      (price * 9n) / 10n
    );
  });
  ...
  it("Controller: commit to mint nft", async function () {
    ...
    await expect(txCommit)
      ...
      .withArgs(
        ...
        tokenId,
        price,
        BigInt((blockData?.timestamp || 0) + commitDuration),
        false
      );
    ...
    await bic
      .connect(wallet1)
      .approve(handlesController.target, ethers.parseEther("1"));
    const newCurrentTime = await ethers.provider
      .getBlock("latest")
    ...
    const newDataHash = await handlesController.getRequestHandleOp(
      ...
      newNextHour as any,
      newCurrentTime as any
    );
    const newSignature = await wallet3.signMessage(
      ethers.getBytes(newDataHash)
    );
    ...
    const blockDataAtClaim = await ethers.provider.getBlock(
      txClaim.blockNumber
    );
  ...
  it("Controller: mint nft and create auction", async function () {
    ...
    const signature = await wallet3.signMessage(ethers.getBytes(dataHash));
    const txRequestHandle = await handlesController
      .connect(wallet1)
      .requestHandle(
        {
          receiver: wallet1.address,
    ...
    await expect(txRequestHandle)
      ...
      .withArgs(
        usernameHandles.target,
        handlesController.target,
        mintName,
        price,
        BigInt(MintType.AUCTION)
      );

    const tokenId = await usernameHandles.getTokenId(mintName);
    expect(await usernameHandles.ownerOf(tokenId)).to.equal(
      testMarketplace.target
    );
    ...
    const collectDataHash = await handlesController.getCollectAuctionPayoutOp(
      auctionId,
      ethers.parseEther("1"),
      [wallet3.address],
      [1000]
    );
    const collectSignature = await wallet3.signMessage(
      ethers.getBytes(collectDataHash)
    );
    await handlesController
      .connect(wallet1)
    ...
    expect(await bic.balanceOf(wallet3.address)).to.equal(
      ethers.parseEther("1") / 10n
    );
    expect(await bic.balanceOf(randomWalletAddress)).to.equal(
      (ethers.parseEther("1") * 9n) / 10n
    );
  });
  ...
  it("Controller: mint nft and create auction but burn it because bid fail and can create again", async function () {
    ...
    await handlesController.burnHandleMintedButAuctionFailed(
      usernameHandles.target,
      mintName
    );
  ...
  describe("Controller: updateCollectsDenominator", async function () {
    it("Controller: should be update successful CollectsDenominator", async function () {
      const currentCollectsDenominator =
        await handlesController.collectsDenominator();
      ...
      const nextCollectsDenominator =
        await handlesController.collectsDenominator();
      expect(nextCollectsDenominator).to.equal(newCollectsDenominator);
      const resetCollectsDenominator = 10000n;
      await handlesController.updateCollectsDenominator(
        resetCollectsDenominator
      );
      const finalCollectsDenominator =
        await handlesController.collectsDenominator();
    ...
    it("Controller: should not update CollectsDenominator if not owner", async function () {
      ...
      await expect(
        handlesController
          .connect(wallet2)
          .updateCollectsDenominator(newCollectsDenominator)
  ...
  describe("Controller: withdraw", async function () {
    it("Controller: should withdraw bic successfully", async function () {
      ...
      await handlesController.withdraw(
        bic.target,
        randomWalletAddress,
        ethers.parseEther("1")
      );
    ...
    it("Controller: should not withdraw if not owner", async function () {
      ...
      await expect(
        handlesController
          .connect(wallet2)
          .withdraw(bic.target, randomWalletAddress, ethers.parseEther("1"))
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
  });
});
