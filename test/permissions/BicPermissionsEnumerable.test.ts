...
import { BicPermissionsEnumerable } from "../../typechain-types";

describe("BicPermissionsEnumerable", function () {
  let bicPermissionsEnumerable: BicPermissionsEnumerable;
  ...
  before(async () => {
    const BicPermissionsEnumerable = await ethers.getContractFactory("BicPermissionsEnumerable");
  ...
  describe("Grant and Revoke role", ()=>{
    ...
    it("Should grant role failed with missing the admin-role grant role", async function () {
      ...
      const grantTx = bicPermissionsEnumerable.connect(wallet2).grantRole(RECOVERY_ROLE, walletGranted);

      expect(grantTx).to.be.revertedWith("AccessControl: account");
